...
Below we first compute the distance and direction from Los Angeles (LA) to New York (NY). These are then used to compute the point from LA at that distance in that (initial) direction. Direction changes continuously when traveling along a Great Circle. The final direction, when approaching NY, is also given.  

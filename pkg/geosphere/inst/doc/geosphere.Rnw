...
<<fig=TRUE , echo=TRUE>>=
LAX <- c(-118.40, 33.95)
JFK <- c(-73.78333,  40.63333)
data(wrld)
plot(wrld, type='l')
gc <- greatCircle(LAX, JFK)
lines(gc, lwd=2, col='blue')
gci <- gcIntermediate(LAX, JFK)
lines(gci, lwd=4, col='green')
points(rbind(LAX, JFK), col='red', pch=20, cex=2)
@
...
<<fig=TRUE , echo=TRUE>>=
ml <- gcMaxLat(LAX, JFK)
lat0 <- gcLat(LAX, JFK, lon=0)
lon0 <- gcLon(LAX, JFK, lat=0)
plot(wrld, type='l')
lines(gc, lwd=2, col='blue')
...
f <- function(lon){gcLat(LAX, JFK, lon)}
...
Points of intersection of two Great Circles:

<<fig=TRUE , echo=TRUE>>=
SFO <- c(-118, 36)
AMS <- c(4.75, 52.31)
gc2 <- greatCircle(AMS, SFO)
plot(wrld, type='l')
lines(gc, lwd=2, col='blue')
lines(gc2, lwd=2, col='green')
int <- gcIntersect(LAX, JFK, SFO, AMS)
points(rbind(int[,1:2], int[,3:4]), col='red', pch=20, cex=2)
@

\section{Great Circle Tracks}

Below we first compute the distance and direction from LAX to JFK. These are then used to compute the point from LAX at that distance in that (initial) direction. Direction changes continuously when traveling along a Great Circle. The final direction, when approaching JFK, is also given.  

<<geo5>>=
d <- distCosine(LAX, JFK)
d
b <- direction(LAX, JFK)
b
destPoint(LAX, b, d)
JFK
finalDirection(LAX, JFK)
@

What if we went off-course and were flying over OFF? The closest point on the planned route (p) can be computed with the alongTrackDistance and destPoint functions. The distance from 'p' to OFF can be computed with the crossTrackDistance function. The light green line represents the along-track distance, and the dark green line represents the cross-track distance.

<<fig=TRUE , echo=TRUE>>=
OFF <- c(-101.18, 42.94)
atd <- alongTrackDistance(LAX, JFK, OFF)
p <- destPoint(LAX, b, atd)
plot(wrld, type='l', xlim=c(-130,-60), ylim=c(22,52))
lines(gci, col='blue', lwd=2)
points(rbind(LAX, JFK), col='red', pch=20, cex=2)
points(OFF[1], OFF[2], pch=20, col='blue', cex=2)
lines(gcIntermediate(LAX, p), col='green', lwd=3)
lines(gcIntermediate(OFF, p), col='dark green', lwd=3)
points(p, pch=20, col='red', cex=2)
crossTrackDistance(LAX, JFK, OFF)
distCosine(p, OFF)
@

...
unsigned char isAntipode(double lon1, double lat1, double lon2, double lat2, double tol) {
	...
	diflon = fabs(lon1 - lon2) - 180;
	diflat = fabs(lat1 + lat2) ;
	return (diflat < tol) & (diflon < tol);
}

void antipodal(int *n, double *lon1, double *lat1, double *lon2, double *lat2, double *tol, int *antipode) {
	int i;
	for(i=0; i < *n; i++) {
		antipode[i] = isAntipode(lon1[i], lat1[i], lon2[i], lat2[i], *tol);
	}
}
...
*/
	...
	if ((lon1 == lon2) & (lat1 == lat2))  {
		...
		while (cont) {
			...
			cont = (abs(lambda-lambdaP) > 1e-12 && iterLimit > 0);
		} 

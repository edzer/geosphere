...
The difference with the 'Vincenty Ellipsoid' method is more pronounced. In the example below (using the default WGS84 ellipsoid), the difference is about 0.3% at very small distances, and 0.15% at larger distances.
